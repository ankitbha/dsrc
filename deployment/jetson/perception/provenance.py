...
def summarise(field_sources: Mapping[str, str]) -> dict[str, Any]:
    ...
    observation is for the caller to decide, and that decision has to be made
    by NAME against `observation_builder.OBS_FIELDS` -- a map with the right
    number of keys but the wrong ones is not coverage, and a count comparison
