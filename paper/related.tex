...
vehicles and a centralized speed planner over cellular
...
centrally~\cite{seo2017traffic,herrera2010evaluation}, and connected-vehicle
standards broadcast per-vehicle position, speed and heading at up to ten
hertz~\cite{sae2020j2735,etsi2019cam,kenney2011dedicated}, which the collective
perception service extends to per-object detections~\cite{etsi2023cpm}. This
design uses neither, because the observation it needs is already published as a
...
feed is a third-party dependency with an unreported lag and different risks, but
it suffices for a minimum viable deployment.

\noindent\textbf{Advisory interfaces and edge perception.}
...
the reasons stated in Section~\ref{sec:intro}. On the sensing side, embedded
detectors and Jetson-class hardware make on-vehicle perception
practical~\cite{redmon2016yolo,nvidia2023jetson,bewley2016sort,yang2024depth},
and the measurements in Section~\ref{sec:eval} are consistent with that
literature. The contribution here is not the detector. It is what the rest of
the system around it has to do to keep the detector's output honest when it is
wrong.
