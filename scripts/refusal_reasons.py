...
CASES = {
    ...
    # coordinated flag day, so it is the case worth reconciling first.
    "rate_cmd without a here object is accepted by both": (RateCommand, RATE_CMD, b""),
    "rate_cmd with a here object is accepted by both": (
        RateCommand, {**RATE_CMD, "here": HERE_QUERY}, b"",
    ),
    "rate_cmd here is not an object": (RateCommand, {**RATE_CMD, "here": "corridor"}, b""),
    "rate_cmd here missing in": (
        RateCommand, {**RATE_CMD, "here": {k: v for k, v in HERE_QUERY.items() if k != "in"}}, b"",
    ),
    "rate_cmd here in is null": (RateCommand, {**RATE_CMD, "here": {**HERE_QUERY, "in": None}}, b""),
    "rate_cmd here in is not a string": (
        RateCommand, {**RATE_CMD, "here": {**HERE_QUERY, "in": 7}}, b"",
    ),
    "rate_cmd here in is blank": (
        RateCommand, {**RATE_CMD, "here": {**HERE_QUERY, "in": "   "}}, b"",
    ),
